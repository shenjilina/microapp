 ...
 * @Date: 2024-01-05 22:49:34
 * @LastEditors: shenjilin-home
 * @LastEditTime: 2024-01-10 22:09:54
 * @Description:
 */
...
export interface GenerateConfigOptions extends GenerateConfigPluginsOptions {
  ...
  /**
   *  打包模式
   * - package - 常规构建，会将所有依赖外部化处理，打包出适用于构建场景的 es, umd, 格式产物
   *              并在构建结束后将产物路径回写入 package.json 的入口字段中。
   * - full - 全量构建。大部分依赖都不做外部化处理，打包出使用于非构建场景的 umd 格式产物。
   *                    不参与 d.ts 的移动，不参与构建完成后的产物路径回写。
  ...
  /**
   * 是否将 d.ts 类型声明文件的产物从集中目录移动到产物目录，并将类型入口会写到 package.json 的 types 字段。
   *
   * 必须在 mode 为 package 时生效。
   *
  ...
  /**
   * 完成构建后，准备回写 package.json 文件前对其对象进行更好的钩子
